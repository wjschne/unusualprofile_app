...
my_theme <- bs_theme(bootswatch = "simplex", base_font = font_google(my_font))
...
# UI ----
ui <- fluidPage(theme = my_theme, shiny::sidebarLayout(
  sidebarPanel(
    ...
    shiny::wellPanel(
      ...
      uiOutput("distances"),
      shiny::fixedRow(
        checkboxInput("sort", value = FALSE, label = "Sort by Score"),
        checkboxInput("color", value = FALSE, label = "Sort by Color")
      )
    ),
    p(
      "Created by ",
      tags$a(target = "_blank", href = "https://wjschne.github.io/", "W. Joel Schneider"),
      "to accompany",
      tags$br(),
  ...
  mainPanel(
    tabsetPanel(
      ...
      tabPanel("Correlations", plotOutput("plot_R")),
      tabPanel(
        "Instructions",
        ...
        p(
          "Click the “Tests\" tab. In each row, enter test score names, means, and standard deviations."
        ),
        img(src = "tests.png", alt = "Test Characteristics", title = "Test Characteristics"),
        h4("Test Correlations"),
        p(
          "Click the \"Correlations\" tab. Enter test correlations in each cell."
        ),
        ...
        tags$ul(
          ...
          tags$li(
            "3: Composite Predictor. That is, the composite score is a linear combination of 2 or more dependent variables."
          ),
        ),
        img(src = "distances.png", alt = "Distances", title = "Distances"),
        h4("Save and Upload the New File"),
        tags$ul(
          tags$li("Save the file, giving it an informative name."),
...
server <- function(input, output) {
  ...
  readxl::read_xlsx(path = fp(), sheet = "scores1")  
  ...
  d_composite <- reactive(readxl::read_xlsx(path = fp(), sheet = "composites"))
  ...
  # Colors
  d_color <- reactive(
    readxl::read_xlsx(path = fp(), sheet = "colors") |>
      rename(Distance = `Distance Name`) |>
      mutate(Distance = fct_inorder(Distance)) |>
      pivot_longer(
        cols = -Distance,
...
filter(d_scores(), `Person ID` == input$inputperson) |>
      pivot_longer(cols = -`Person ID`, values_to = "Score", names_to = "Test") |>
  ...
  output$plot_R <- renderPlot({
    as.data.frame(R()) |>
      ...
      pivot_longer(cols = -test, names_to = "test_x", values_to = "r") |>
  ...
  mycondmaha <- reactive({
    ...
    v_all <- unique(c(v_dep, v_ind, v_ind_composites))
  ...
  output$condplot <- renderPlot({
    ...
    break_mu <- max(x$mu)
    minor_break_width <- ifelse(break_width %% 3 == 0, 1 / 3, 1 / 2)
    ...
    if ("maha" %in% class(x)) {
      d_stats <-
        ...
        mutate(
          Color = ifelse(Color == 0, NA, Color) |> factor(),
          fcol = scales::dscale(
            Color,
            scales::viridis_pal(
              option = "D",
      ...
      d <- x$d_dep %>%
        ...
        tidyr::pivot_longer(cols = -id, names_to = "Variable", values_to = "Score") %>%
      ...
      ggplot(d, aes(Variable, z)) +
        ...
        scale_x_discrete(
          ...
          labels = \(x) str_wrap(x, 10, whitespace_only = FALSE)
        ) +
      ...
      label_independent <- paste0(
        ...
        formatC(x$dM_ind, digits = 2, format = "f"),
        "\",italic(p)==\"",
      ...
      label_dependent <- paste0(
        ...
        formatC(x$dM_dep, digits = 2, format = "f"),
        "\",italic(p)==\"",
      ...
      d |>
        ...
        geom_text(
          mapping = aes(
            ...
            label = if_else(
              ...
              "",
              paste0("italic(c*p)=='", proportion_round(cp), "'")
            )
          ),
          vjust = 2.3,
          size = 3,
        ...
        geom_text(
          mapping = aes(
            color = id,
            label = paste0("italic(phantom(c)*p)=='", proportion_round(p), "'")
          ),
          vjust = 1.3,
          size = 3,
          parse = TRUE,
